...
description: UQL is the smartest ORM for TypeScript with serializable queries, no codegen, and a unified API across SQL and MongoDB.
template: splash
hero:
  title: UQL
  tagline: The smartest TypeScript ORM. Serializable queries, no codegen, and a unified API across Postgres, MySQL, SQLite, MongoDB, and the edge.
  actions:
    - text: Get Started
...
- **Fastest, even vs. query builders.** Fastest in [all 8 categories](/benchmark) of our [open benchmark](https://github.com/rogerpadilla/ts-orm-benchmark): ~2.1× faster on average than the runner-up, and quicker than raw builders like Knex and Kysely, reaching 3.5M ops/s on simple SELECTs and DELETEs.
