...
export class User {
  @Id() id: number;
  @Field({ onDelete: () => new Date() }) deletedAt: Date;
}
...
```ts title="UQL"
// Backend: one handler mounts on Hono, Elysia, Next.js, Bun, Deno, Workers, SvelteKit, Astro...
import { createFetchHandler } from 'uql-orm/http';

const handler = createFetchHandler({ include: [User] });

// ...or use the Express adapter
...
**In practice:** Every ORM in this comparison except UQL requires you to build your own API bridge per model. UQL ships a framework-agnostic [HTTP transport core](/extensions-http) (REST routes, transactions, authorization hooks) plus a typed [browser client](/extensions-browser), connecting frontend to database out of the box.
