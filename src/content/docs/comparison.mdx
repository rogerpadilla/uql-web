...
**In practice:** Drizzle ships native pgvector helpers (`cosineDistance`, `l2Distance`) for PostgreSQL. The other ORMs treat vector similarity as an "edge case" requiring raw SQL and database-specific syntax (like `<->`). UQL handles it as a native, type-safe operator across dialects.
