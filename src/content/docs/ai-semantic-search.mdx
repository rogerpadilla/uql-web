...
`$project` adds the computed score to each row (here: `distance`), so your app can filter, rank, or inspect relevance. Annotate the result with the exported `WithDistance<Article, 'distance'>` helper to type the projected field.
