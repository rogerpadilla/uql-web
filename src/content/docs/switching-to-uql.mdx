...
<Tabs>
  ...
  <TabItem label="Drizzle">
    ...
    const results = await db.select()
      .from(articles)
      .where(eq(articles.category, 'docs'))
      .orderBy(cosineDistance(articles.embedding, queryEmbedding))
      .limit(5);
    ```
  </TabItem>
  ...
  <TabItem label="TypeORM">
    ```ts
    const results = await manager.createQueryBuilder(Article, 'article')
      .where('article.category = :category', { category: 'docs' })
      .orderBy('article.embedding <=> :vector')
      .setParameter('vector', queryEmbedding)
      .limit(5)
      .getMany();
    ```
  </TabItem>
  <TabItem label="MikroORM">
    ...
    const results = await em.createQueryBuilder(Article, 'a')
      .where({ category: 'docs' })
      .orderBy({ [cosineDistance('embedding', queryEmbedding)]: 'ASC' })
      .limit(5)
      .getResult();
    ```
  </TabItem>
  <TabItem label="UQL">
    ...
    const results = await querier.findMany(Article, {
      $where: { category: 'docs' },
      $sort: {
        embedding: { $vector: queryEmbedding, $distance: 'cosine' }
      },
      $limit: 5
    });
    ```
...
Only UQL keeps the metadata filter and vector ranking in one type-safe JSON query that runs unchanged across pgvector, CockroachDB, MariaDB, SQLite, and MongoDB Atlas. The others are tied to one database or step outside the type-safe API: Prisma and TypeORM drop to raw SQL, Drizzle and MikroORM use Postgres-only pgvector helpers, and Mongoose needs an Atlas-specific aggregation. See [AI & RAG](/ai-semantic-search) for the full ingestion-to-retrieval walkthrough.
