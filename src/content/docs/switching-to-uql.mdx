...
Most ORMs charge a "tax" on the developer: build-time complexity, runtime verbosity, or state-management overhead. UQL is built to avoid all three.
...
Drizzle is an excellent thin wrapper, but complex queries often become a forest of `eq()`, `and()`, and `sql` templates. UQL uses a JSON-native declarative API that is easier to read, easier to write, and — crucially — transportable over the network.
