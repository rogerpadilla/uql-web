...
Only UQL keeps the metadata filter and vector ranking in one type-safe JSON query that runs unchanged across pgvector, CockroachDB, MariaDB, SQLite, and MongoDB Atlas. The others drop to raw SQL (Prisma, TypeORM, MikroORM), a Postgres-only helper (Drizzle), or an Atlas-specific aggregation (Mongoose). See [AI & RAG](/ai-semantic-search) for the full ingestion-to-retrieval walkthrough.
